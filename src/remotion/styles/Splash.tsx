...
export const SplashToken = memo(function SplashToken({
  ...
  if (role === "accent") {
    ...
    // Borrow Playfair Display italic if the base face is not already script/serif
    if (!roleFontFamily?.toString().includes("playfair") && !roleFontFamily?.toString().includes("caveat")) {
      roleFontFamily = "var(--bolo-font-playfair)";
    }
  ...
  const dropShadow = highlight > 0.01
